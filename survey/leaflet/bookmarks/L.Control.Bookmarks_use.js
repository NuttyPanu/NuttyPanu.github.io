...
(function (global, factory) {
  ...
  var FormPopup = L.Popup.extend( /** @lends FormPopup.prototype */ {
    ...
    options: {
      ...
      templateOptions: {
        ...
        inputPlaceholder: 'โปรดระวังการกรอกข้อมูลส่วนบุคคล',
      ...
      template: '<form class="{{ formClass }}">' +
        '<div class="input-group" style="width: 100%;margin-left: 0px;margin-right: 0px; padding: 0px;"><input type="text" name="bookmark-name" ' +
    ...
    _onMenuButtonClick: function(evt) {
      console.log('click_onMenuButtonClick');
  ...
  var Bookmarks = L.Control.extend( /**  @lends Bookmarks.prototype */ {
    ...
    options: {
      ...
      bookmarkTemplate: '<li title="{{data.name}}"" class="{{ itemClass }}" data-id="{{ data.id }}">' +
    ...
    onAdd: function(map) {
      ...
      //container.attr('title','Bookmarks');
    ...
    expand: function() {
      ...
      if(stattmpPoint != 0){
        ...
        //bmMarker.clearLayers();
        map._layers[tmpPoint].remove();
        stattmpPoint = 0;
        tmpPoint= 0;
        //map.removeLayer(bmMarker);
      }
      */
      //markerGroup.clearLayers();
      /*
    ...
    _createMarker: function(coords, bookmark) {
      ...
      bmMarker = new L.Marker(coords, {
        icon: this.options.icon || new L.icon({popupAnchor:  [0, 5]}),
        riseOnHover: true
      });
      ...
      */
      ...
      markerGroup.addLayer(bmMarker);
      //bmMarker.addTo(markerGroup);
      markerGroup.addTo(map);
      /*
      markerGroup.addLayer(new L.Marker(coords, {
        icon: this.options.icon || new L.Icon.Default(),
        riseOnHover: true
      ...
      */
      ...
      console.log(bmMarker._leaflet_id);
      if(stattmpPoint == 0){
        tmpPoint= parseInt(bmMarker._leaflet_id);
        stattmpPoint=1;
      }
      else{
      ...
      */
      ...
      var removeIfRemoved = this._getOnRemoveHandler(bookmark, bmMarker);
    ...
    _onBookmarkClick: function(evt) {
      ...
      console.log('_onBookmarkClick: '+evt.target);
