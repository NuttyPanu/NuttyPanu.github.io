...
!(function() {
    ...
    var formatDistance = function(d) {
        ...
        if (d < 100) {
            return numberWithCommas(d.toFixed(1)) + ' ' + unit;
        } else {
            return numberWithCommas(Math.round(d)) + ' ' + unit;
        }
    }
    ...
    var formatArea = function(a) {
        ...
        if (a < 100) {
            return numberWithCommas(a.toFixed(1)) + ' ' + unit;
          // return numberWithCommas(a) + ' ' + unit;
        } else {
           // return numberWithCommas(Math.round(a)) + ' ' + unit;
           return numberWithCommas(Math.round(a)) + ' ' + unit;
        }
    }
	...
	var circlePerimeter =function (d){
        var length=(2*( Math.PI)  *d);
		var unit;
            if (length > 1000) {
                length = length / 1000;
                unit = 'km';
        ...
        return numberWithCommas(length.toFixed(1))+' '+unit;
	};
        ...
        showMeasurements: function(options) {
            ...
            this._measurementOptions = L.extend({
                ...
                lang: {
                    ...
                    segmentLength: 'ความยาวแต่ละช่วง'
                }
            }, options || {});
    ...
    L.Circle.include({
        ...
        updateMeasurements: function() {
            ...
            if (options.showArea) {
				...
				var perimiter=circlePerimeter(this.getRadius());
                L.marker.measurement(latLng, 
                   'P: '+perimiter+'<br>A: '+ formatter(area), options.lang.totalArea, 0, options)
                    .addTo(this._measurementLayer);
            }
        }
    })    
